...
def test_step2():
    assert checkout(f"cd {folderout}; 7z x arx2.7z -o{folderext2} -y",
                    "Everything is Ok"), "test2 FAIL"


def test_step10():
    res = subprocess.run("crc32 /home/user/out/arx2.7z",
                         shell=True,
                         stdout=subprocess.PIPE,
                         encoding='utf-8')
    assert checkout(f"cd {folderout}; 7z h arx2.7z",
                    f"{(res.stdout).rstrip().upper()}"), "test10 FAIL"
